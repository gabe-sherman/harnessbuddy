...
def test_action_required_is_reported_as_a_failed_result(tmp_path: Path) -> None:
    """Needing a person to act is an expected result of a repair attempt, not an error, so
    it comes back as a failed result carrying the reason and the agent's own report."""
...
def test_a_failed_library_repair_records_why_compile_commands_are_absent(tmp_path: Path) -> None:
    """BuildExplorationResult documents compile_commands_path and compile_commands_error as
    mutually exclusive; without this the printed line reads "not captured (None)"."""
    (tmp_path / "work").mkdir()
    with patch(
        "harnessbuddy.library_builder.agents.run_agent_streaming",
...
# post-agent validation checks real artifacts in every environment: the gate mounts the
# workspace, so install/ and out/ land on the host whether the build ran there or in a
# container. There is nothing left to infer from the agent's transcript.
...
def test_library_agent_publishes_the_repaired_script(tmp_path: Path) -> None:
    """The agent edits build_library.sh in place, and the gate it just passed ran that
    script from nothing — so the script that passed is the script that ships."""
...
def test_marker_quoted_in_tool_output_does_not_fail_a_verified_build(tmp_path: Path) -> None:
    ...
    substring match reported action_required and failed the run.
    """
    workdir = _repaired_workdir(tmp_path)
    with patch(
        "harnessbuddy.library_builder.agents.run_agent_streaming",
