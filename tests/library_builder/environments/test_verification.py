...
def test_command_builder_matches_what_run_verification_executes(
    ...
    """The command builder has to stay in sync with what actually runs, since callers use it to
    report a reproduce-it command without paying to run it."""
...
def test_oss_fuzz_verification_runs_the_gate_in_a_container(tmp_path: Path) -> None:
    """Same assertions, different place: the container wrapper builds the image and mounts the
    workspace, then runs the one gate script inside it."""
    with _patched_runner() as mock_run:
...
def test_a_relative_workspace_is_resolved_before_the_gate_sees_it(
    ...
    """Every gate script cds to the path it is handed and _run also sets cwd to it, so a
    relative path is applied twice: `cd .harnessbuddy/foo` from inside .harnessbuddy/foo. The
    CLI passes exactly such a path, since default_state_dir is relative."""
...
def test_environment_unavailable_pattern_matches_gate_output(tmp_path: Path) -> None:
    """The gate's `docker build` writes daemon-unreachable errors into the merged output stream,
    so a caller telling an unavailable environment from a build failure reads `output` rather
    than `stderr`, which run_command_streaming always leaves empty."""
